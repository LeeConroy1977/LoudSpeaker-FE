...
const SearchBarCard = ({ article }) => {
  ...
  return (
    <Link to={`/articles/${article.article_id}`}>
      <div
        className="flex justify-between items-center w-full h-[84px]   sm:w-full sm:h-[80px]  border-b border-gray-200 dark:border-primary sm:p-2 cursor-pointer"
        onClick={toggleSearchOpen}
      >
        <div className="flex flex-col justify-start items-start w-[70%] sm:w-[76%] h-full">
          <div className="text-[0.675rem] mt-2 sm:mt-0 font-semibold dark:text-darkTextPrimary">
            {title}
          </div>
          <div className="text-[0.6rem] mt-1 mb-2 dark:text-darkTextPrimary">
            {reducedBody}...
          </div>
        </div>
        <img
